function twix_obj = mapVBVD_tb(filename,readType,varargin)

%  Reads Siemens raw .dat file from VB/VD MRI raw data.
%
%  Requires twix_map_obj_tb.m
%
...
%                          twix_map_obj_tb.m now stores the memory position of
...
%     .vop:           vop rf data
%     .calib:         gradient calibration lines % tbenkert
%
...
for s=1:NScans
    ...
    twix_obj{s}.image         = twix_map_obj_tb(arg,'image',filename,version,readType,s,rstraj);
    twix_obj{s}.noise         = twix_map_obj_tb(arg,'noise',filename,version,s,readType);
    twix_obj{s}.phasecor      = twix_map_obj_tb(arg,'phasecor',filename,version,readType,s,rstraj);
    twix_obj{s}.phasestab     = twix_map_obj_tb(arg,'phasestab',filename,version,readType,s,rstraj);
    twix_obj{s}.phasestabRef0 = twix_map_obj_tb(arg,'phasestab_ref0',filename,version,readType,s,rstraj);
    twix_obj{s}.phasestabRef1 = twix_map_obj_tb(arg,'phasestab_ref1',filename,version,readType,s,rstraj);
    twix_obj{s}.refscan       = twix_map_obj_tb(arg,'refscan',filename,version,readType,s,rstraj);
    twix_obj{s}.refscanPC     = twix_map_obj_tb(arg,'refscan_phasecor',filename,version,readType,s,rstraj);
    twix_obj{s}.refscanPS     = twix_map_obj_tb(arg,'refscan_phasestab',filename,version,readType,s,rstraj);
    twix_obj{s}.refscanPSRef0 = twix_map_obj_tb(arg,'refscan_phasestab_ref0',filename,version,readType,s,rstraj);
    twix_obj{s}.refscanPSRef1 = twix_map_obj_tb(arg,'refscan_phasestab_ref1',filename,version,readType,s,rstraj);
    twix_obj{s}.RTfeedback    = twix_map_obj_tb(arg,'rtfeedback',filename,version,readType,s,rstraj);
    twix_obj{s}.vop           = twix_map_obj_tb(arg,'vop',filename,version,readType,s); % tx-array rf pulses
    twix_obj{s}.calib         = twix_map_obj_tb(arg,'calib',filename,version,readType,s); % tbenkert
    ...
    if arg.bReadCalib
        clear tmpMdh
        isCurrScan = logical( mask.MDH_CALIB );
        for f = fieldnames( mdh ).'
            tmpMdh.(f{1}) = mdh.(f{1})( isCurrScan, : );
        end
        twix_obj{s}.calib.readMDH( tmpMdh, filePos(isCurrScan) );
    end
    ...
    for scan = { 'image', 'noise', 'phasecor', 'phasestab', ...
                 ...
                 'refscanPSRef1', 'RTfeedback', 'vop', 'calib'}
        f = scan{1};
...
mask.MDH_CALIB              = min(bitand(mdh.aulEvalInfoMask(:,2), 2^(35-32)),1); % tbenkert
...
noImaScan = (   mask.MDH_ACQEND             | mask.MDH_RTFEEDBACK   | mask.MDH_HPFEEDBACK       ...
              ...
              | (mask.MDH_PATREFSCAN & ~mask.MDH_PATREFANDIMASCAN)  | mask.MDH_CALIB);
