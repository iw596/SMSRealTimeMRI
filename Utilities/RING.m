function S = RING(data,angles,NCalib)
for i = 1:NCalib
    ray_pick = i;
    ray_1 = squeeze(data(:,ray_pick));
    theta_1 = angles(ray_pick);
    d_theta = theta_1 - angles;
    [~, ray_pick_partner] = min(abs(abs(d_theta) -pi/2));
    theta_1_partner = angles(ray_pick_partner);
    ray_1_partner = squeeze(data(:,ray_pick_partner));

    % Get the center of k-space
    ray_1_im = fftshift(ifft(ray_1),1);
    ray_1_partner_im = fftshift(ifft(ray_1_partner),1);
    % Remove outerpart of image
    sx = size(ray_1_im,1);
    drop = round(sx*0.2);
